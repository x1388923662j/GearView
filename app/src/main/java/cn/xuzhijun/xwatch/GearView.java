...
public class GearView extends View {
	...
	private int     gearToothCount = 112;
	...
	private void preDrawGear() {
		...
		if (showSideHole) {
			int c = 5;
			for (int i = 0; i < c; i++) {
				PointF p = getRadioPoint(45 + (i * 360 / c), r / 2);
		...
		paintFill.setXfermode(xfermode);
		gearCanvas.drawCircle(0, 0, r / 6 / 2, paintFill);
		paintFill.setXfermode(null);
		gearCanvas.drawCircle(0, 0, r / 6 / 2, paintStroke);
		paintFill.setColor(gearColor);
	...
	private static PointF getRadioPoint(double radio, float r) {
		PointF point = new PointF();
		double x, y;
